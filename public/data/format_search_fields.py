...
def format_search_fields():
    ...
    df = pd.read_json(os.path.join(sys.path[0], f'output/{folder_path}/{folder_path}_combined.json'), orient='records')
    ...
    df_search.to_json(os.path.join(sys.path[0], f'output/{folder_path}/{folder_path}_search_fields.json'), orient='records')
